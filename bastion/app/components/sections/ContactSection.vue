<template>
  <section
    id="contact"
    class="scroll-mt-20 px-4 py-20 sm:px-6 sm:py-24 lg:px-8"
  >
    <div class="mx-auto max-w-2xl text-center">
      <h2 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">
        Contact
      </h2>
      <p class="mt-4 text-lg text-slate-400">
        Want to know more or work with us?
      </p>

      <div class="mt-12 rounded-2xl border border-slate-800/50 bg-slate-900/30 p-10 backdrop-blur-sm">
        <!-- Email avec icône (plus grand) -->
          <a
    href="mailto:bastion.space.solution@gmail.com"
    class="group inline-flex max-w-full items-center gap-2 rounded-lg bg-blue-600 px-3 py-2.5 text-xs font-medium text-white transition hover:scale-105 hover:bg-blue-500 sm:gap-3 sm:px-6 sm:py-3 sm:text-base"
  >
    <svg class="size-4 shrink-0 sm:size-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
    </svg>
    <span class="min-w-0 truncate">bastion.space.solution@gmail.com</span>
  </a>

        <!-- Informations complémentaires (agrandies) -->
        <div class="mt-8 space-y-3 text-base text-slate-400">
          <p class="flex items-center justify-center gap-2">
            <span class="inline-block size-2 rounded-full bg-green-400"></span>
            Usually responds within 24h
          </p>
          <p class="text-sm text-slate-500">
            Or connect with us on
          </p>
          <!-- Liens sociaux (icônes agrandies) -->
          <div class="flex justify-center gap-6">
            <a
              href="https://www.linkedin.com/company/bastion-space"
              target="_blank"
              rel="noopener noreferrer"
              class="text-slate-400 transition hover:text-blue-400"
            >
              <svg class="size-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
            </a>
            
            <a
              href="https://github.com/bastion-space"
              target="_blank"
              rel="noopener noreferrer"
              class="text-slate-400 transition hover:text-white"
            >
              <svg class="size-8" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.399 1.02 0 2.046.133 3.003.399 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
              </svg>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
</script>